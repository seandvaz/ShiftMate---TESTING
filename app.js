...
const perthShiftLabels={PN:'Perth Assist Arvo',PA:'Perth Afternoon',PD:'Perth Assist Day',PM:'Perth Morning',PRD:'Perth Roaming Day',UD:'Underground Day',UN:'Underground Arvo'};
  ...
  // Shift details are temporarily moved into the modal on small screens. Use
  // this accessor whenever a roster control may currently live in that modal.
  ...
  function allowedRosterScanCodes(valid=validScanCodes()){
    const home=String(current.settings.homeLine||'').trim().toUpperCase();
    if(!home)return valid;
    const homeCodes=new Set(Object.entries(SHIFT_DATA||{}).filter(([code,data])=>{
      const line=String(data?.line||'').toUpperCase(),designated=String(data?.designatedLine||'').toUpperCase();
      return valid.has(String(code).toUpperCase())&&(line===home||designated===home);
    }).map(([code])=>String(code).toUpperCase()));
    // If the saved home line has no matching reference data, keep scanning rather
    // than silently rejecting every code.
    return homeCodes.size?homeCodes:valid;
  }
  ...
  function scanAssignmentsFromWords(words,periodEnd){
    const valid=validScanCodes(),allowed=allowedRosterScanCodes(valid),start=addRosterDays(periodEnd,-13),assign=Array(14).fill('');
      ...
      if(gridLike){
        ...
        if(anchors.length>=5){
          ...
          for(let i=0;i<14;i++){
            ...
            const candidates=cleanWords.filter(w=>allowed.has(w.token)&&Math.abs(cx(w)-x0)<=colHalf&&Math.abs(cy(w)-targetY)<=yTol)
  ...
  async function captureRosterScan(){
    ...
    try{
      ...
      // The visible roster header is usually in the centre-right of a portrait
      // phone photo of a laptop screen. Crop it tightly so its small PE text is
      // enlarged before OCR, instead of being diluted by the surrounding room.
      const periodHeader=makeScanVariant(canvas,Math.round(W*.42),Math.round(H*.30),Math.round(W*.58),Math.round(H*.32),3200,1.18);
      ...
      if(!pe){
        results.push(await ocrRosterRegion(T,periodHeader,'Reading pay period',status));
        pe=chooseScanPeriodEnd(results);
      }
  ...
  function exportBackup(){
    ...
    const payload={
      app:'PTA ShiftMate',
      version:'2.7.4-corporate-console',
