...
const perthShiftLabels={PN:'Perth Assist Arvo',PA:'Perth Afternoon',PD:'Perth Assist Day',PM:'Perth Morning',PRD:'Perth Roaming Day',UD:'Underground Day',UN:'Underground Arvo'};
  ...
  const allNetworkShiftOptions=(selected='',date=null)=>{
    ...
    const codes=Object.keys(SHIFT_DATA||{})
      .filter(code=>{
        const data=SHIFT_DATA[code];
        if(data?.leaveType)return false;
        if(code==='SA')return true;
  ...
  const effectiveShiftCode=card=>{
    ...
    if(main===OFFLINE_CODE)return card.querySelector('.offline-shift-code')?.value||card.dataset.effectiveShiftCode||'';
    return main;
  };
    ...
    current.days.forEach((row,i)=>{
      ...
      card.querySelector('.shift-code').onchange=()=>{
        ...
        if(main===OFFLINE_CODE){
          ...
          // OFFLINE is a UI trigger only; do not feed it into PayCalc.
        ...
        if(line){
          line.value=current.settings.homeLine;
      ...
      const manualTime=()=>{
        const data=SHIFT_DATA[effectiveShiftCode(card)];
      ...
      const considerBookOff=()=>{const diff=bookOffDifference(effectiveShiftCode(card),date,card.querySelector('.start-time').value,card.querySelector('.finish-time').value);if(diff)openBookOffChoice(card,date)};
  ...
  function recalculate(){
    ...
    latestResult.dayTotals.forEach((row,i)=>{
      ...
      if(row.start&&row.finish&&SHIFT_DATA[current.days[i].code]){
        ...
        const face=card.querySelector('.shift-face-code');
        if(face)face.textContent=effectiveShiftCode(card)?(/^\d+$/.test(effectiveShiftCode(card))?`T${effectiveShiftCode(card)}`:effectiveShiftCode(card)):'OFF';
