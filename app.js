...
const perthShiftLabels={PN:'Perth Assist Arvo',PA:'Perth Afternoon',PD:'Perth Assist Day',PM:'Perth Morning',PRD:'Perth Roaming Day',UD:'Underground Day',UN:'Underground Arvo'};
  ...
  async function generateShiftSwap(){
    ...
    try{
      ...
      const pages=[];
      entries.forEach(({request:a,partner:b})=>{const c=document.createElement('canvas');c.width=1241;c.height=1754;const ctx=c.getContext('2d');ctx.drawImage(template,0,0);ctx.textBaseline='middle';const clear=[[171,247,498,292],[772,247,1013,292],[281,357,486,392],[832,357,1013,392],[281,407,486,440],[832,407,1013,440],[281,457,486,490],[832,457,1013,490],[281,505,486,540],[832,505,1013,540],[171,1142,498,1185],[772,1142,1013,1185],[281,1252,486,1290],[832,1252,1013,1290],[281,1302,486,1340],[832,1302,1013,1340],[281,1350,486,1387],[832,1350,1013,1387],[281,1399,486,1436],[832,1399,1013,1436]];ctx.fillStyle='#fff';clear.forEach(r=>ctx.fillRect(...r));ctx.fillStyle='#111';const text=(x,y,v,size=25)=>{if(v===undefined||v===null||v==='')return;ctx.font=`${size}px Arial`;ctx.fillText(String(v),x,y)};const fill=(y,name,service,date,shift,start,finish)=>{text(180,y,name,25);text(785,y,service,25);text(290,y+84,date?swapFmtDate(date):'',24);text(290,y+132,shift,25);text(290,y+180,start,25);text(290,y+228,finish,25)};fill(270,reqName,reqService,a.date,a.shift,a.start,a.finish);fill(1160,partnerName,partnerService,b.date,b.shift,b.start,b.finish);if($('#swapContingent')?.checked)text(600,584,'✓',28);if($('#swapNeutralCost')?.checked)text(548,842,'✓',28);const jpeg=c.toDataURL('image/jpeg',.96);pages.push({width:c.width,height:c.height,bytes:Uint8Array.from(atob(jpeg.split(',')[1]),x=>x.charCodeAt(0))})});
