...
const perthShiftLabels={PN:'Perth Assist Arvo',PA:'Perth Afternoon',PD:'Perth Assist Day',PM:'Perth Morning',PRD:'Perth Roaming Day',UD:'Underground Day',UN:'Underground Arvo'};
  ...
  async function generateShiftSwap(){
    ...
    try{
      ...
      const pages=[];
      entries.forEach(({request:a,partner:b})=>{const c=document.createElement('canvas');c.width=1241;c.height=1754;const ctx=c.getContext('2d');ctx.drawImage(template,0,0);ctx.fillStyle='#111';ctx.textBaseline='middle';const clear=[[170,245,500,300],[770,245,1015,300],[280,355,500,398],[830,355,1015,398],[280,405,490,445],[830,405,1015,445],[280,455,490,495],[830,455,1015,495],[280,503,490,545],[830,503,1015,545],[170,1140,500,1190],[770,1140,1015,1190],[280,1250,500,1295],[830,1250,1015,1295],[280,1300,490,1345],[830,1300,1015,1345],[280,1348,490,1392],[830,1348,1015,1392],[280,1397,490,1440],[830,1397,1015,1440],[650,568,1010,598]];clear.forEach(r=>ctx.fillRect(...r));const text=(x,y,v,size=25)=>{if(v===undefined||v===null||v==='')return;ctx.font=`${size}px Arial`;ctx.fillText(String(v),x,y)};const fill=(y,name,service,date,shift,start,finish)=>{text(180,y,name,25);text(785,y,service,25);text(290,y+84,date?swapFmtDate(date):'',24);text(290,y+132,shift,25);text(290,y+180,start,25);text(290,y+228,finish,25)};fill(270,reqName,reqService,a.date,a.shift,a.start,a.finish);fill(1160,partnerName,partnerService,b.date,b.shift,b.start,b.finish);if($('#swapContingent')?.checked)text(600,584,'✓',28);if($('#swapNeutralCost')?.checked)text(548,842,'✓',28);const jpeg=c.toDataURL('image/jpeg',.96);pages.push({width:c.width,height:c.height,bytes:Uint8Array.from(atob(jpeg.split(',')[1]),x=>x.charCodeAt(0))})});
