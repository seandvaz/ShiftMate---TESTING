...
const perthShiftLabels={PN:'Perth Assist Arvo',PA:'Perth Afternoon',PD:'Perth Assist Day',PM:'Perth Morning',PRD:'Perth Roaming Day',UD:'Underground Day',UN:'Underground Arvo'};
  ...
  function scanAssignmentsFromWords(words,periodEnd){
    ...
    const dateWords=[];cleanWords.forEach(w=>{
      const candidates=[year-1,year,year+1].map(y=>parseOcrDateToken(w.text,y)).filter(Boolean);
      const d=candidates.find(x=>x>=addRosterDays(start,-1)&&x<=addRosterDays(periodEnd,1));
      if(d)dateWords.push({w,d});
    });
