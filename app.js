...
const perthShiftLabels={PN:'Perth Assist Arvo',PA:'Perth Afternoon',PD:'Perth Assist Day',PM:'Perth Morning',PRD:'Perth Roaming Day',UD:'Underground Day',UN:'Underground Arvo'};
  ...
  let swapMode='single';
  const swapState={requestDates:[],partnerDates:[],pendingAdd:false};
  ...
  const swapCycle=date=>date?rosterTimeline().get(localISO(date))?.cycle:null;
  ...
  const populateSwapShift=(select,date,line='',preferred='')=>{
    if(!select)return '';
    const codes=swapCodesForDate(date,line),rosterCode=swapEffectiveCode(swapRosterRow(date));
    ...
    select.value=wanted; return wanted;
  };
  const isSwapBlankRequestDate=date=>{const row=swapRosterRow(date);return !row||!row.code||row.type==='Off'||!row.entered};
  const swapPEForDate=date=>{const cycle=swapCycle(date);if(cycle?.startDate){const end=addRosterDays(parseDate(cycle.startDate),13);return swapFmtPE(end)}return '';};
  const updateSwapWarnings=()=>{
    const rd=swapDateValue('#swapRequestDate'),pd=swapDateValue('#swapPartnerDate');
    const reqWarn=$('#swapRequestBlankWarning');
    if(reqWarn)reqWarn.hidden=!rd||!isSwapBlankRequestDate(rd);
    const peWarn=$('#swapPartnerPEWarning');
    if(peWarn){const rpe=swapPEForDate(rd),ppe=swapPEForDate(pd);peWarn.hidden=!(rd&&pd&&rpe&&ppe&&rpe!==ppe);if(!peWarn.hidden)peWarn.textContent=`This date is in a different fortnight (${ppe}) than the requesting employee's selected date (${rpe}).`;}
  };
  const fillSwapSide=(side,date)=>{
    ...
    const select=$(`#${prefix}Shift`),preferred=select?.value||'';
    const code=populateSwapShift(select,date,side==='request'?current.settings.homeLine:'',preferred);
    ...
    if(row&&side==='request'){
      ...
      if(effective)populateSwapShift(select,date,current.settings.homeLine,effective);
  ...
  const renderSwapCalendar=(selectedKey='')=>{
    ...
    const focusMonth=new Date(focus.getFullYear(),focus.getMonth(),1),first=new Date(focusMonth.getFullYear(),focusMonth.getMonth()-2,1),last=new Date(focusMonth.getFullYear(),focusMonth.getMonth()+3,1);
    wrap.innerHTML='';
    for(let monthDate=new Date(first);monthDate<=last;monthDate=new Date(monthDate.getFullYear(),monthDate.getMonth()+1,1)){
      const year=monthDate.getFullYear(),month=monthDate.getMonth(),daysInMonth=new Date(year,month+1,0).getDate(),section=document.createElement('section');
      section.className='calendar-month-block';section.dataset.month=`${year}-${String(month+1).padStart(2,'0')}`;
      const header=document.createElement('h3');header.className='calendar-scroll-month';header.textContent=monthDate.toLocaleDateString('en-AU',monthDate.getFullYear()===new Date().getFullYear()?{month:'short'}:{month:'short',year:'numeric'});section.appendChild(header);
      const weekdays=document.createElement('div');weekdays.className='calendar-weekdays';weekdays.innerHTML='<span>Sun</span><span>Mon</span><span>Tue</span><span>Wed</span><span>Thu</span><span>Fri</span><span>Sat</span>';section.appendChild(weekdays);
      const grid=document.createElement('div');grid.className='calendar-grid';
      for(let i=0;i<monthDate.getDay();i++){const blank=document.createElement('div');blank.className='calendar-day blank';grid.appendChild(blank)}
      for(let day=1;day<=daysInMonth;day++){
        const date=new Date(year,month,day),key=localISO(date),entry=entries.get(key),row=entry?.row,candidateProjection=date>=today?rosterProjectionForDate(date):null,actualCycleStart=entry?.cycle?.startDate?parseDate(entry.cycle.startDate):null,actualFortnight=Boolean(actualCycleStart),projection=!actualFortnight?candidateProjection:null,isActual=actualFortnight;
        const label=isActual?rosterDisplayCode(entry):(projection?.working?(projection.shiftType==='Morning'?'Morn':'Arvo'):''),period=isActual?actualShiftPeriod(entry,date):(projection?.shiftType||'Off'),periodClass=period==='Morn'||period==='Morning'?'shift-morn':period==='Arvo'?'shift-arvo':'shift-off';
        const b=document.createElement('button');b.type='button';b.dataset.key=key;b.className=`calendar-day ${isActual?'actual':'projected'} ${periodClass} ${key===localISO(today)?'today':''} ${key===selectedKey?'selected':''}`;
  ...
  function selectSwapDate(date){
    ...
    if(swapMode==='multi'){
      if(!swapState.requestDates.some(d=>localISO(d)===key)){swapState.requestDates.push(date);swapState.partnerDates.push(new Date(date));}
      swapState.pendingAdd=false;renderSwapEntries();
    }else{
  ...
  const swapEntryCard=(index,date,partnerDate)=>`<div class="swap-entry-card" data-swap-index="${index}"><div class="swap-entry-head"><strong>Swap ${index+1}</strong><button type="button" class="ghost swap-remove-date" data-swap-index="${index}">Remove</button></div><div class="swap-entry-date-grid"><label>Your date<input class="swap-entry-date" data-swap-side="request" data-swap-index="${index}" type="date" value="${date?localISO(date):''}"></label><label>Replacement date<input class="swap-entry-date" data-swap-side="partner" data-swap-index="${index}" type="date" value="${partnerDate?localISO(partnerDate):''}"></label></div><div class="swap-entry-columns"><div><span class="eyebrow">Your shift</span><select class="swap-entry-shift" data-swap-side="request" data-swap-index="${index}"></select><div class="swap-time-grid"><label>Start<input class="swap-entry-start" data-swap-side="request" data-swap-index="${index}" type="time"></label><label>Finish<input class="swap-entry-finish" data-swap-side="request" data-swap-index="${index}" type="time"></label></div></div><div><span class="eyebrow">Replacement shift</span><select class="swap-entry-shift" data-swap-side="partner" data-swap-index="${index}"></select><div class="swap-time-grid"><label>Start<input class="swap-entry-start" data-swap-side="partner" data-swap-index="${index}" type="time"></label><label>Finish<input class="swap-entry-finish" data-swap-side="partner" data-swap-index="${index}" type="time"></label></div></div></div><small class="swap-entry-warning" data-swap-index="${index}" hidden></small></div>`;
  function hydrateSwapEntry(card,index,date,partnerDate){
    if(!date){card.querySelectorAll('select,input[type=time]').forEach(el=>el.value='');return}
    const req=card.querySelector('[data-swap-side="request"].swap-entry-shift'),partner=card.querySelector('[data-swap-side="partner"].swap-entry-shift'),row=swapRosterRow(date),code=swapEffectiveCode(row);
    populateSwapShift(req,date,current.settings.homeLine,code);populateSwapShift(partner,partnerDate||date,'','');
    const rt=swapTimes(date,req.value),pt=swapTimes(partnerDate||date,partner.value);
    ...
    const warning=card.querySelector('.swap-entry-warning');if(warning){warning.hidden=!isSwapBlankRequestDate(date);warning.textContent="Check you've selected the correct date";}
    const rpe=swapPEForDate(date),ppe=swapPEForDate(partnerDate||date);if(rpe&&ppe&&rpe!==ppe){warning.hidden=false;warning.textContent=`Replacement date is in a different fortnight (${ppe}) than your selected date (${rpe}).`;warning.classList.add('swap-pe-warning');}
  }
  function renderSwapEntries(){
    const wrap=$('#swapMultiEntries');if(!wrap)return;wrap.innerHTML='';
    swapState.requestDates.forEach((date,i)=>{const holder=document.createElement('div');holder.innerHTML=swapEntryCard(i,date,swapState.partnerDates[i]);const card=holder.firstElementChild;wrap.appendChild(card);hydrateSwapEntry(card,i,date,swapState.partnerDates[i]);});
    $$('#swapMultiEntries .swap-remove-date').forEach(b=>b.onclick=()=>{const i=Number(b.dataset.swapIndex);swapState.requestDates.splice(i,1);swapState.partnerDates.splice(i,1);renderSwapEntries();});
    $$('#swapMultiEntries .swap-entry-date').forEach(input=>input.onchange=()=>{const i=Number(input.dataset.swapIndex),d=parseDate(input.value),side=input.dataset.swapSide;if(d){if(side==='partner')swapState.partnerDates[i]=d;else swapState.requestDates[i]=d;renderSwapEntries();renderSwapCalendar(localISO(d));}});
    $$('#swapMultiEntries .swap-entry-shift').forEach(sel=>sel.onchange=()=>{const i=Number(sel.dataset.swapIndex),side=sel.dataset.swapSide,date=swapState.requestDates[i],t=swapTimes(date,sel.value),card=sel.closest('.swap-entry-card');if(!card)return;card.querySelector(`[data-swap-side="${side}"] .swap-entry-start`).value=t[0]||'';card.querySelector(`[data-swap-side="${side}"] .swap-entry-finish`).value=t[1]||'';});
  }
  const syncSwapMode=()=>{
    const multi=swapMode==='multi',roster=swapMode==='roster';
    $('#swapSingleFields')?.toggleAttribute('hidden',multi);$('#swapMultiFields')?.toggleAttribute('hidden',!multi);$('#swapRosterNote')?.toggleAttribute('hidden',!roster);$('#swapCalendarWrap')?.toggleAttribute('hidden',false);
    if(roster){const d=swapDateValue('#swapRequestDate')||new Date(),pe=swapFmtPE(d);$('#swapRequestShift').innerHTML=`<option value="${pe}">${pe}</option>`;$('#swapPartnerShift').innerHTML=`<option value="${pe}">${pe}</option>`;$('#swapRequestStart').value='';$('#swapRequestFinish').value='';$('#swapPartnerStart').value='';$('#swapPartnerFinish').value='';}
    else if(!multi)syncSwapTimes();else renderSwapEntries();
    updateSwapWarnings();
  };
  const openShiftSwap=()=>{
    const sheet=$('#shiftSwapSheet');if(!sheet)return;swapMode='single';swapState.requestDates=[];swapState.partnerDates=[];swapState.pendingAdd=false;
  ...
  async function generateShiftSwap(){
    const reqName=String($('#swapRequestingName')?.textContent||'').trim(),reqService=String(current.settings.serviceNumber||'').trim(),partnerName=String($('#swapPartnerName')?.value||'').trim(),partnerService=String($('#swapPartnerService')?.value||'').trim();
    if(!reqName||!reqService){toast('Add your name and service number in Settings');return}if(!partnerName||!partnerService){toast('Enter the replacement employee details');return}
    let entries=[];
    if(swapMode==='multi'){
      if(!swapState.requestDates.length){toast('Add at least one swap date');return}
      const cards=$$('#swapMultiEntries .swap-entry-card');cards.forEach((card,i)=>{const date=swapState.requestDates[i],partnerDate=swapState.partnerDates[i]||date,rq=card.querySelector('[data-swap-side="request"].swap-entry-shift')?.value||'',ps=card.querySelector('[data-swap-side="partner"].swap-entry-shift')?.value||'';if(!date)return;entries.push({request:{date,shift:rq,start:card.querySelector('[data-swap-side="request"].swap-entry-start')?.value||'',finish:card.querySelector('[data-swap-side="request"].swap-entry-finish')?.value||''},partner:{date:partnerDate,shift:ps,start:card.querySelector('[data-swap-side="partner"].swap-entry-start')?.value||'',finish:card.querySelector('[data-swap-side="partner"].swap-entry-finish')?.value||''}})});
    }else{const rd=swapDateValue('#swapRequestDate'),pd=swapDateValue('#swapPartnerDate');if(!rd||!pd){toast('Select both dates');return}if(swapMode==='roster'){entries=[{request:{date:null,shift:swapFmtPE(rd),start:'',finish:''},partner:{date:null,shift:swapFmtPE(pd),start:'',finish:''}}]}else{entries=[{request:{date:rd,shift:$('#swapRequestShift').value,start:$('#swapRequestStart').value,finish:$('#swapRequestFinish').value},partner:{date:pd,shift:$('#swapPartnerShift').value,start:$('#swapPartnerStart').value,finish:$('#swapPartnerFinish').value}}]}}
    if(!entries.length){toast('Add a swap date');return}
    try{
      const template=await new Promise((res,rej)=>{const im=new Image();im.onload=()=>res(im);im.onerror=()=>rej(new Error('Template failed to load'));im.src='./shift-change-form-template.png'}),pages=[];
      entries.forEach(({request:a,partner:b})=>{const c=document.createElement('canvas');c.width=1241;c.height=1754;const ctx=c.getContext('2d');ctx.drawImage(template,0,0);ctx.fillStyle='#fff';
        [[995,560,1138,610],[535,808,575,850]].forEach(r=>ctx.fillRect(...r));ctx.fillStyle='#111';ctx.textBaseline='middle';
        const text=(x,y,v,size=25)=>{if(v===undefined||v===null||v==='')return;ctx.font=`${size}px Arial`;ctx.fillText(String(v),x,y)};
        const fill=(y,name,service,date,shift,start,finish)=>{text(180,y,name,25);text(785,y,service,25);text(290,y+84,date?swapFmtDate(date):'',24);text(290,y+132,shift,25);text(290,y+180,start,25);text(290,y+228,finish,25)};
        fill(270,reqName,reqService,a.date,a.shift,a.start,a.finish);fill(1160,partnerName,partnerService,b.date,b.shift,b.start,b.finish);const fillRequested=(y,date,shift,start,finish)=>{text(835,y+90,date?swapFmtDate(date):'',24);text(835,y+138,shift,25);text(835,y+186,start,25);text(835,y+234,finish,25)};fillRequested(270,b.date,b.shift,b.start,b.finish);fillRequested(1160,a.date,a.shift,a.start,a.finish);if($('#swapContingent')?.checked)text(548,584,'✓',28);if($('#swapOthers')?.value)text(900,584,$('#swapOthers').value,20);if($('#swapNeutralCost')?.checked)text(548,842,'✓',28);
        const jpeg=c.toDataURL('image/jpeg',.96);pages.push({width:c.width,height:c.height,bytes:Uint8Array.from(atob(jpeg.split(',')[1]),x=>x.charCodeAt(0))})});
      const blob=swapJpegPdf(pages),url=URL.createObjectURL(blob),w=window.open(url,'_blank');if(!w){const a=document.createElement('a');a.href=url;a.download=`ShiftMate-Shift-Swap-${localISO(entries[0].request.date||new Date())}.pdf`;document.body.appendChild(a);a.click();a.remove()}setTimeout(()=>URL.revokeObjectURL(url),60000);toast(pages.length>1?`${pages.length} shift swap forms generated`:'Shift swap form generated');
    }catch(error){console.error(error);toast('Could not generate shift swap form')}
  }

  // Critical controls are delegated at document level so one optional initializer
  // cannot make core Home/documentation actions appear dead.
  document.addEventListener('click',e=>{
    const t=e.target.closest?.('#homeGenerateLeave,#homeGenerateSwap,#homeNewCycle,#generateLeaveDocument,#leaveFormCancel,#generateShiftSwap,#shiftSwapCancel,#shiftSwapCancelTop');
    if(!t)return;
    e.preventDefault();e.stopImmediatePropagation();
    if(t.id==='homeGenerateLeave')return openLeaveFormFromHome();
    if(t.id==='homeGenerateSwap')return openShiftSwap();
    if(t.id==='homeNewCycle')return startNextFortnight();
    if(t.id==='generateLeaveDocument')return generateLeaveForm(pendingLeaveDocument);
    if(t.id==='leaveFormCancel')return closeLeaveFormSheet();
    if(t.id==='generateShiftSwap')return generateShiftSwap().catch(error=>{console.error(error);toast('Could not generate shift swap form')});
    return closeShiftSwap();
  },true);
...
const syncViewport=()=>{const vv=window.visualViewport;document.documentElement.style.setProperty('--app-vh',`${vv?.height||window.innerHeight}px`);const nav=document.querySelector('.bottom-nav');if(nav)document.documentElement.style.setProperty('--bottom-nav-h',`${Math.ceil(nav.getBoundingClientRect().height)}px`);requestAnimationFrame(()=>{if(nav){nav.style.bottom='0px';nav.style.left='0px';nav.style.right='0px'}sizeCalendarViewport()})};
  ...
  if($('#shiftSwapCancelTop'))$('#shiftSwapCancelTop').onclick=closeShiftSwap;
  if($('#shiftSwapSheet'))$('#shiftSwapSheet').onclick=e=>{if(e.target===$('#shiftSwapSheet'))closeShiftSwap()};
  $$('#shiftSwapSheet [data-swap-mode]').forEach(b=>b.onclick=()=>{swapMode=b.dataset.swapMode;$$('#shiftSwapSheet [data-swap-mode]').forEach(x=>x.classList.toggle('active',x===b));if(swapMode==='multi'&&!swapState.requestDates.length)swapState.pendingAdd=true;syncSwapMode()});
  ...
  if($('#swapPartnerDate'))$('#swapPartnerDate').onchange=()=>{const d=swapDateValue('#swapPartnerDate');if(d){fillSwapSide('partner',d);updateSwapWarnings();}};
  if($('#addSwapDate'))$('#addSwapDate').onclick=()=>{swapState.pendingAdd=true;toast('Select a date from the calendar to add the swap');};
  ...
  // Swipe down to cancel the Shift Swap sheet, matching Leave Form behaviour.
  const swapSheet=$('#shiftSwapSheet'),swapSheetCard=swapSheet?.querySelector('.shift-swap-card');
  if(swapSheetCard){
    let sy=0,sx=0,drag=false;
    swapSheetCard.addEventListener('touchstart',e=>{
      if(swapSheet.hidden||e.touches.length!==1||e.target.closest('button,input,select,textarea'))return;
      const r=swapSheetCard.getBoundingClientRect();
      if(e.touches[0].clientY-r.top>92)return;
      sy=e.touches[0].clientY;sx=e.touches[0].clientX;drag=true;
    },{passive:true});
    const endSwapSwipe=e=>{
      if(!drag)return; drag=false;
      const t=e.changedTouches[0],dy=t.clientY-sy,dx=Math.abs(t.clientX-sx);
      if(dy>80&&dx<60&&swapSheetCard.scrollTop<=2)closeShiftSwap();
    };
    swapSheetCard.addEventListener('touchend',endSwapSwipe,{passive:true});
    swapSheetCard.addEventListener('touchcancel',()=>{drag=false},{passive:true});
  }
