...
const perthShiftLabels={PN:'Perth Assist Arvo',PA:'Perth Afternoon',PD:'Perth Assist Day',PM:'Perth Morning',PRD:'Perth Roaming Day',UD:'Underground Day',UN:'Underground Arvo'};
  ...
  // Home documentation actions use delegated handlers so they remain responsive even if a later optional UI initializer fails.
  document.addEventListener('click',e=>{
    const leave=e.target.closest?.('#homeGenerateLeave');
    if(leave){e.preventDefault();e.stopPropagation();openLeaveFormFromHome();return;}
    const swap=e.target.closest?.('#homeGenerateSwap');
    if(swap){e.preventDefault();e.stopPropagation();openShiftSwap();return;}
  },true);
