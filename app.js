...
(() => {
  ...
  function openLeaveFormSheet(key,entry){
    ...
    const sheet=$('#leaveFormSheet'),summary=$('#leaveFormRangeSummary');
    const label=block.leaveType==='annual'?'Annual Leave':block.leaveType==='sick'?'Personal Leave':block.leaveType==='lsl'?'Long Service Leave':'Leave';
    $('#leaveFormSheetTitle').textContent=`${label} application`;
    summary.innerHTML=`<strong>Review leave details</strong><small>Dates and hours are pre-filled from the saved roster and can be adjusted before generating the PDF.</small>`;
    $('#leaveFormStart').value=localISO(block.start);$('#leaveFormEnd').value=localISO(block.end);$('#leaveFormHours').value=block.hours.toFixed(1);
    $('#leaveFormComments').value='';$('#leaveContactable').value='yes';$('#leaveEvidence').value='no';sheet.hidden=false;
  }
  ...
  function generateLeaveForm(block){
    if(!block)return;
    const startValue=$('#leaveFormStart')?.value,endValue=$('#leaveFormEnd')?.value,hoursValue=Number($('#leaveFormHours')?.value);
    ...
    block={...block,start:adjustedStart,end:adjustedEnd,hours:Number.isFinite(hoursValue)?Math.max(0,hoursValue):block.hours};
    const name=String(current.settings.employeeName||'').trim(),service=String(current.settings.serviceNumber||'').trim();
    const home=window.ROSTER_LINES?.[current.settings.homeLine]||current.settings.homeLine||'';
    const rowName=leaveFormRow(block),comments=String($('#leaveFormComments')?.value||'').trim()||(block.leaveType==='lwop'?'Leave Without Pay':'');
    const contactable=$('#leaveContactable')?.value==='yes',evidence=$('#leaveEvidence')?.value==='yes',today=new Date();
    const rows=['Annual Leave','Long Service Leave','Illness or injury','Providing care/support to family','Unanticipated matter requiring immediate attention','Bereavement Leave','Day In Lieu','Paid Parental Leave','Unpaid Parental Leave','Military Leave','Union Leave','Additional Day of Leave (Easter Sunday)','Other'];
    const bodyRows=rows.map(r=>`<tr><td>${esc(r)}</td><td>${r===rowName?fmtFormDate(block.start):''}</td><td>${r===rowName?fmtFormDate(block.end):''}</td><td>${r===rowName?block.hours.toFixed(1):''}</td><td>${r===rowName?esc(comments):''}</td></tr>`).join('');
    const html=`<!doctype html><html><head><meta charset="utf-8"><title>Leave Application - ${esc(name||'Transit Officer')}</title><style>
      @page{size:A4;margin:10mm}body{font-family:Arial,sans-serif;color:#111;margin:0;font-size:11px}.toolbar{margin-bottom:10px}.toolbar button{padding:8px 14px;margin-right:8px}h1{text-align:center;font-size:19px;margin:4px 0}.sub{text-align:center;color:#267f95;font-weight:bold;margin-bottom:8px}.section{background:#2f8ca1;color:white;font-weight:bold;padding:4px 6px;margin-top:6px}table{width:100%;border-collapse:collapse}td,th{border:1px solid #777;padding:4px;vertical-align:top}th{font-size:10px}.label{font-weight:bold}.no-border td{border:0;padding:2px 4px}.checks{display:flex;gap:30px;padding:8px;border:1px solid #777}.endorse td{height:18px}@media print{.toolbar{display:none}}
      </style></head><body><div class="toolbar"><button onclick="window.print()">Print / Save PDF</button><button onclick="window.close()">Close</button></div>
      <div style="text-align:center;font-size:10px">OFFICIAL</div><h1>APPLICATION FOR LEAVE – TRANSIT OFFICERS</h1><div class="sub">Once completed please return to HR Services - Personnel and Payroll</div>
      <div class="section">SECTION 1: EMPLOYEE DETAILS</div><table><tr><td colspan="4"><span class="label">Name:</span> ${esc(name)}</td></tr><tr><td><span class="label">Position:</span> TRANSIT OFFICER</td><td colspan="2"><span class="label">Service Number:</span> ${esc(service)}</td></tr><tr><td><span class="label">Home Line:</span> ${esc(home)}</td><td colspan="2"><span class="label">Branch:</span> Security Services</td></tr></table>
      <div class="section">SECTION 2: LEAVE BOOKING</div><table><thead><tr><th>TYPE</th><th>START OF LEAVE</th><th>END OF LEAVE</th><th>HOURS</th><th>COMMENTS</th></tr></thead><tbody>${bodyRows}</tbody></table>
      <div class="section">SECTION 3: AVAILABILITY &amp; EVIDENCE</div><div class="checks"><div><b>I can be contacted while on leave if necessary</b><br>Yes ${contactable?'☒':'☐'} &nbsp; No ${contactable?'☐':'☒'}</div><div><b>Evidence attached to leave application</b><br>Yes ${evidence?'☒':'☐'} &nbsp; No ${evidence?'☐':'☒'}</div></div>
      <div class="section">SECTION 4: ENDORSEMENT &amp; APPROVALS</div><table class="endorse"><tr><td><b>Leave Approved:</b><br>Yes ☐ &nbsp; No ☐</td><td><b>Comments:</b></td></tr><tr><th>Employee</th><th>Supervisor / Manager</th></tr><tr><td><b>Name:</b> ${esc(name)}</td><td><b>Name:</b></td></tr><tr><td><b>Signature:</b></td><td><b>Signature:</b></td></tr><tr><td><b>Date:</b> ${fmtFormDate(today)}</td><td></td></tr><tr><th>Roster Office Use Only</th><th>Roster Clerk / Checking Officer</th></tr><tr><td><b>Name:</b></td><td><b>Name:</b></td></tr><tr><td><b>Signature:</b></td><td><b>Signature:</b></td></tr></table><div style="text-align:right;margin-top:8px;font-size:9px">OFFICIAL · Effective as of 1 July 2026</div></body></html>`;
    const w=window.open('','_blank');if(!w){alert('Allow pop-ups for ShiftMate TEST to generate the leave form.');return}w.document.open();w.document.write(html);w.document.close();
    $('#leaveFormSheet').hidden=true;toast('Leave form generated');
  }
