...
(() => {
  ...
  const opts=(selected,line,date=null)=>{
    ...
    return codes.map(code=>{const label=!code?'Off / no shift':(/^\d+$/.test(code)?`Tango ${code}`:SHIFT_DATA[code].name);return `<option value="${code}" ${code===selected?'selected':''}>${label}</option>`}).join('');
  };
...
let activeShiftDetailsCard=null;
let shiftDetailsTouchStartY=0;

function closeShiftDetailsSheet(){
  const sheet=document.getElementById('shiftDetailsSheet');
  if(sheet)sheet.hidden=true;
  document.body.classList.remove('modal-open');
  activeShiftDetailsCard=null;
}

function populateShiftDetailsSheet(card,date){
  const fields=document.getElementById('shiftDetailsFields');
  const title=document.getElementById('shiftDetailsTitle');
  if(!fields||!title||!card)return;

  activeShiftDetailsCard=card;
  title.textContent=date.toLocaleDateString('en-AU',{weekday:'short',day:'numeric',month:'short'});

  const code=card.querySelector('.shift-code')?.value||'';
  const rowType=card.querySelector('.type')?.value||card.dataset.type||'Rostered';
  const workedLine=card.querySelector('.worked-roster-line')?.value||card.dataset.workedRosterLine||'';
  const offline=card.querySelector('.offline-arrangement')?.value||'Directed / rostered offline';
  const partner=card.querySelector('.partner')?.value||'';
  const higher=card.querySelector('.higher-duties')?.value||'No';
  const start=card.querySelector('.start-time')?.value||'';
  const finish=card.querySelector('.finish-time')?.value||'';
  const personalReason=card.querySelector('.personal-leave-reason')?.value||'illness';
  const paidLeaveHours=card.querySelector('.paid-leave-hours')?.value||'';
  const annualPortion=card.querySelector('.annual-leave-portion')?.value||'0';
  const phBenefit=card.querySelector('.public-holiday-benefit')?.value||'leave';

  const optionHTML=(sel)=>sel ? Array.from(sel.options).map(o=>`<option value="${o.value.replace(/"/g,'&quot;')}" ${o.value===sel.value?'selected':''}>${o.textContent}</option>`).join('') : '';

  fields.innerHTML=`
    <div class="form-grid shift-details-global-grid">
      <label>Worked line
        <select data-field=".worked-roster-line">${optionHTML(card.querySelector('.worked-roster-line'))}</select>
      </label>
      <label>Offline arrangement
        <select data-field=".offline-arrangement">${optionHTML(card.querySelector('.offline-arrangement'))}</select>
      </label>
      <label>Type
        <select data-field=".type">${optionHTML(card.querySelector('.type'))}</select>
      </label>
      <label>Partner
        <input data-field=".partner" value="${partner.replace(/"/g,'&quot;')}" placeholder="Optional"/>
      </label>
      <label>Higher duties
        <select data-field=".higher-duties">${optionHTML(card.querySelector('.higher-duties'))}</select>
      </label>
      <label>Start
        <input data-field=".start-time" type="time" value="${start}"/>
      </label>
      <label>Finish
        <input data-field=".finish-time" type="time" value="${finish}"/>
      </label>
      ${card.querySelector('.personal-leave-reason')?`<label>Personal leave reason
        <select data-field=".personal-leave-reason">${optionHTML(card.querySelector('.personal-leave-reason'))}</select>
      </label>`:''}
      ${card.querySelector('.paid-leave-hours')?`<label>Paid leave hours
        <input data-field=".paid-leave-hours" type="number" step="0.01" value="${paidLeaveHours}" placeholder="Use shift hours"/>
        <small class="field-note">Optional override for partial-day leave.</small>
      </label>`:''}
      ${card.querySelector('.annual-leave-portion')?`<label>Annual leave portion (hrs)
        <input data-field=".annual-leave-portion" type="number" step="0.01" value="${annualPortion}"/>
        <small class="field-note">For a mixed sick/personal + annual leave day.</small>
      </label>`:''}
      ${card.querySelector('.public-holiday-benefit')?`<label>Public holiday worked benefit
        <select data-field=".public-holiday-benefit">${optionHTML(card.querySelector('.public-holiday-benefit'))}</select>
      </label>`:''}
    </div>`;

  fields.querySelectorAll('[data-field]').forEach(input=>{
    input.addEventListener('change',()=>{
      const target=card.querySelector(input.dataset.field);
      if(target){
...
function openShiftDetailsSheet(card,date){
  const sheet=document.getElementById('shiftDetailsSheet');
  if(!sheet)return;
  populateShiftDetailsSheet(card,date);
  sheet.hidden=false;
  document.body.classList.add('modal-open');
}

function installShiftDetailsSheet(){
  const sheet=document.getElementById('shiftDetailsSheet');
  if(!sheet||sheet.dataset.installed==='true')return;
  sheet.dataset.installed='true';

  const close=()=>closeShiftDetailsSheet();
  document.getElementById('shiftDetailsClose')?.addEventListener('click',close);

  sheet.addEventListener('click',e=>{
    if(e.target===sheet)close();
  });

  const card=sheet.querySelector('.shift-details-sheet-card');
  card?.addEventListener('touchstart',e=>{
    if(e.touches.length!==1)return;
    shiftDetailsTouchStartY=e.touches[0].clientY;
  },{passive:true});
  card?.addEventListener('touchend',e=>{
    if(e.changedTouches.length!==1)return;
    const dy=e.changedTouches[0].clientY-shiftDetailsTouchStartY;
    const scroll=sheet.querySelector('.shift-details-scroll');
    if(dy>85 && (!scroll || scroll.scrollTop<=2))close();
  },{passive:true});
}
...
function buildRoster(){
    ...
    current.days.forEach((row,i)=>{
      ...
      card.innerHTML=`<div class="roster-module-housing"><div class="roster-module-face"><div class="day-head"><div><b>${date.toLocaleDateString('en-AU',{weekday:'short',day:'numeric',month:'short'})}</b><small>Day ${i+1}</small></div><span class="day-pay">$0.00</span></div>
      <div class="day-main">
        ...
        <button type="button" class="ot-toggle ${row.type==='Picked-up OT'?'active':''}"><span class="ot-lamp"></span>OT</button><button type="button" class="details-button" aria-label="Shift details">•••</button>
      </div>
      ...
      card.querySelector('.details-button').onclick=()=>openShiftDetailsSheet(card,date);
