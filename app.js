...
(() => {
  ...
  const opts=(selected,line,date=null)=>{
    ...
    return codes.map(code=>{const label=!code?'Off / no shift':(/^\d+$/.test(code)?`Train Riding ${code}`:SHIFT_DATA[code].name);return `<option value="${code}" ${code===selected?'selected':''}>${label}</option>`}).join('');
  };
