  ...
  async function generateLeaveForm(block){
    ...
    try{
      ...
      entries.forEach(({request:a,partner:b})=>{
        ...
        // Text is rendered on a transparent overlay. No white rectangles are drawn over the official form.
        const overlay=document.createElement('canvas');overlay.width=c.width;overlay.height=c.height;
        const octx=overlay.getContext('2d');octx.textBaseline='middle';octx.fillStyle='#111';
        const drawValue=(x,y,v,size=25)=>{if(v===undefined||v===null||v==='')return;octx.font=`bold ${size}px Arial`;octx.fillText(String(v),x,y)};
        const fmtTime=v=>String(v||'').replace(':','');
        // Requesting employee: identity plus the four shift fields on both sides.
        ...
        drawValue(285,465,fmtTime(a.start),25);drawValue(830,465,fmtTime(b.start),25);
        drawValue(285,515,fmtTime(a.finish),25);drawValue(830,515,fmtTime(b.finish),25);
        // Replacement employee: identity plus the four shift fields on both sides, reversed for the swap.
        ...
        drawValue(285,1370,fmtTime(b.start),25);drawValue(830,1370,fmtTime(a.start),25);
        drawValue(285,1420,fmtTime(b.finish),25);drawValue(830,1420,fmtTime(a.finish),25);
