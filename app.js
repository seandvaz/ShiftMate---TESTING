...
const perthShiftLabels={PN:'Perth Assist Arvo',PA:'Perth Afternoon',PD:'Perth Assist Day',PM:'Perth Morning',PRD:'Perth Roaming Day',UD:'Underground Day',UN:'Underground Arvo'};
  ...
  const swapFmtPE=d=>d?`PE${String(d.getDate()).padStart(2,'0')}${String(d.getMonth()+1).padStart(2,'0')}${String(d.getFullYear())}`:'';
  const swapCodeForDate=(date,line,preferred='')=>{
    if(!date)return preferred||'';
    const group=PayCalc.dayGroup(date.getDay());
    const candidates=Object.keys(SHIFT_DATA).filter(code=>{
      const data=SHIFT_DATA[code];
      if(!data||data.leaveType||code==='SA'&&false)return false;
      if(line&&code!=='SA'&&data.line!==line)return false;
      const t=data.times?.[group]||['',''];
      return Boolean(t[0]&&t[1]);
    }).sort((a,b)=>a.localeCompare(b,undefined,{numeric:true}));
  ...
  const populateSwapShift=(select,date,line,preferred='')=>{
    ...
    const roster= date?rosterTimeline().get(localISO(date))?.row:null;
    const rosterCode=roster?.code||'';
    select.value=(preferred&&codes.includes(preferred)?preferred:(rosterCode&&codes.includes(rosterCode)?rosterCode:(codes[0]||'')));
  };
  const swapTimes=(date,code)=>{
    const data=SHIFT_DATA[code],t=data?.times?.[date?PayCalc.dayGroup(date.getDay()):0]||['',''];return t;
  };
  const syncSwapTimes=()=>{
    const rd=swapDateValue('#swapRequestDate'),pd=swapDateValue('#swapPartnerDate');
    const rc=$('#swapRequestShift')?.value||'',pc=$('#swapPartnerShift')?.value||'';
    const rt=swapTimes(rd,rc),pt=swapTimes(pd,pc);
    if($('#swapRequestStart'))$('#swapRequestStart').value=rt[0]||'';if($('#swapRequestFinish'))$('#swapRequestFinish').value=rt[1]||'';
    if($('#swapPartnerStart'))$('#swapPartnerStart').value=pt[0]||'';if($('#swapPartnerFinish'))$('#swapPartnerFinish').value=pt[1]||'';
  };
  const syncSwapMode=()=>{
    const multi=swapMode==='multi',roster=swapMode==='roster';
    ['#swapRequestEndWrap','#swapPartnerEndWrap'].forEach(id=>{const el=$(id);if(el)el.hidden=!multi});
    if($('#swapRequestDateLabel'))$('#swapRequestDateLabel').textContent=multi?'Start date':'Date';
    if($('#swapPartnerDateLabel'))$('#swapPartnerDateLabel').textContent=multi?'Start date':'Date';
    const note=$('#swapMultiNote');if(note)note.hidden=!multi;
    ['#swapRequestDate','#swapRequestEndDate','#swapPartnerDate','#swapPartnerEndDate'].forEach(id=>{if($(id))$(id).disabled=roster});
    ['#swapRequestShift','#swapPartnerShift'].forEach(id=>{if($(id))$(id).disabled=roster});
    if(roster){
      const pe=swapFmtPE(swapDateValue('#swapRequestDate')||new Date());
      if($('#swapRequestShift')){$('#swapRequestShift').innerHTML=`<option value="${pe}">${pe}</option>`;$('#swapRequestShift').value=pe}
      if($('#swapPartnerShift')){$('#swapPartnerShift').innerHTML=`<option value="${pe}">${pe}</option>`;$('#swapPartnerShift').value=pe}
      ['#swapRequestStart','#swapRequestFinish','#swapPartnerStart','#swapPartnerFinish'].forEach(id=>{if($(id))$(id).value='' });
    }else{
      const rd=swapDateValue('#swapRequestDate'),pd=swapDateValue('#swapPartnerDate');
      populateSwapShift($('#swapRequestShift'),rd,current.settings.homeLine,$('#swapRequestShift')?.value||'');
      populateSwapShift($('#swapPartnerShift'),pd,'',$('#swapPartnerShift')?.value||'');
      syncSwapTimes();
    }
  };
  const openShiftSwap=()=>{
    const sheet=$('#shiftSwapSheet');if(!sheet)return;
    swapMode='single';
    const today=new Date(),rd=rosterTimeline().get(localISO(today))?.row;
    const d=rd?.code?today:today;
    if($('#swapRequestingName'))$('#swapRequestingName').textContent=current.settings.employeeName||'—';
    if($('#swapRequestingService'))$('#swapRequestingService').textContent=`Service No: ${current.settings.serviceNumber||'—'}`;
    if($('#swapRequestDate'))$('#swapRequestDate').value=localISO(d);
    if($('#swapPartnerDate'))$('#swapPartnerDate').value=localISO(d);
    if($('#swapRequestEndDate'))$('#swapRequestEndDate').value=localISO(d);
    if($('#swapPartnerEndDate'))$('#swapPartnerEndDate').value=localISO(d);
    if($('#swapPartnerName'))$('#swapPartnerName').value='';if($('#swapPartnerService'))$('#swapPartnerService').value='';
    if($('#swapContingent'))$('#swapContingent').checked=false;if($('#swapNeutralCost'))$('#swapNeutralCost').checked=false;if($('#swapOthers'))$('#swapOthers').value='';
    if($('#swapOthersWrap'))$('#swapOthersWrap').hidden=true;
    $$('#shiftSwapSheet [data-swap-mode]').forEach(b=>b.classList.toggle('active',b.dataset.swapMode==='single'));
    populateSwapShift($('#swapRequestShift'),d,current.settings.homeLine,rd?.code||'');
    populateSwapShift($('#swapPartnerShift'),d,'','');
    syncSwapMode();sheet.hidden=false;
  };
  const closeShiftSwap=()=>{if($('#shiftSwapSheet'))$('#shiftSwapSheet').hidden=true};
  const swapJpegPdf=pages=>{
    const enc=s=>new TextEncoder().encode(s),cat=(...a)=>{let n=a.reduce((x,b)=>x+b.length,0),o=new Uint8Array(n),p=0;for(const b of a){o.set(b,p);p+=b.length}return o};
    const objs=[null],offsets=[0];
    pages.forEach((pg,i)=>{
      const pageId=3+i*3,imgId=4+i*3,contentId=5+i*3;
      objs[pageId]=enc(`<< /Type /Page /Parent 2 0 R /MediaBox [0 0 ${pg.width} ${pg.height}] /Resources << /XObject << /Im${i} ${imgId} 0 R >> >> /Contents ${contentId} 0 R >>`);
      objs[imgId]=cat(enc(`<< /Type /XObject /Subtype /Image /Width ${pg.width} /Height ${pg.height} /ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter /DCTDecode /Length ${pg.bytes.length} >>\nstream\n`),pg.bytes,enc('\nendstream'));
      const stream=`q\n${pg.width} 0 0 ${pg.height} 0 0 cm\n/Im${i} Do\nQ\n`;objs[contentId]=enc(`<< /Length ${stream.length} >>\nstream\n${stream}endstream`);
    });
    const kids=pages.map((_,i)=>`${3+i*3} 0 R`).join(' ');objs[1]=enc('<< /Type /Catalog /Pages 2 0 R >>');objs[2]=enc(`<< /Type /Pages /Kids [${kids}] /Count ${pages.length} >>`);
    const chunks=[enc('%PDF-1.4\n%\xE2\xE3\xCF\xD3\n')];let off=chunks[0].length;
    for(let id=1;id<objs.length;id++){offsets[id]=off;const h=enc(`${id} 0 obj\n`),t=enc('\nendobj\n');chunks.push(h,objs[id],t);off+=h.length+objs[id].length+t.length}
  ...
  async function generateShiftSwap(){
    const reqName=String($('#swapRequestingName')?.textContent||'').trim(),reqService=String(current.settings.serviceNumber||'').trim();
    const partnerName=String($('#swapPartnerName')?.value||'').trim(),partnerService=String($('#swapPartnerService')?.value||'').trim();
    ...
    const rd=swapDateValue('#swapRequestDate'),pd=swapDateValue('#swapPartnerDate');if(!rd||!pd){toast('Select both dates');return}
    if(swapMode==='multi'){
      const re=swapDateValue('#swapRequestEndDate'),pe=swapDateValue('#swapPartnerEndDate');if(!re||!pe||re<rd||pe<pd){toast('Check the multi-day date ranges');return}
      const rn=Math.round((re-rd)/86400000)+1,pn=Math.round((pe-pd)/86400000)+1;if(rn!==pn){toast('Both multi-day ranges must contain the same number of days');return}
    }
    const template=await new Promise((res,rej)=>{const im=new Image();im.onload=()=>res(im);im.onerror=rej;im.src='./shift-change-form-template.png'});
    const pages=[], addPage=(a,b)=>{
      const c=document.createElement('canvas');c.width=1241;c.height=1754;const ctx=c.getContext('2d');ctx.drawImage(template,0,0);
      ctx.fillStyle='#111';ctx.textBaseline='middle';ctx.font='25px Arial';
      // Remove remaining template placeholders inside value areas.
      [[170,245,500,300],[770,245,1015,300],[280,355,500,398],[830,355,1015,398],[280,405,490,445],[830,405,1015,445],[280,455,490,495],[830,455,1015,495],[280,503,490,545],[830,503,1015,545],[170,1140,500,1190],[770,1140,1015,1190],[280,1250,500,1295],[830,1250,1015,1295],[280,1300,490,1345],[830,1300,1015,1345],[280,1348,490,1392],[830,1348,1015,1392],[280,1397,490,1440],[830,1397,1015,1440],[650,568,1010,598]].forEach(r=>ctx.fillRect(...r));
      const text=(x,y,v,size=25)=>{if(v===undefined||v===null||v==='')return;ctx.font=`${size}px Arial`;ctx.fillText(String(v),x,y)};
      const fillSide=(y,name,service,date,shift,start,finish,other=false)=>{text(180,y,name,25);text(785,y,service,25);text(290,y+84,swapFmtDate(date),24);text(290,y+132,shift,25);text(290,y+180,start,25);text(290,y+228,finish,25)};
      // Coordinates are based on the supplied official A4 form rendered at 1241×1754.
      fillSide(270,reqName,reqService,a.date,a.shift,a.start,a.finish);
      text(850,585,other||'',21);
      fillSide(1160,partnerName,partnerService,b.date,b.shift,b.start,b.finish);
      if($('#swapContingent')?.checked){ctx.font='28px Arial';text(600,584,'✓',28)}
      if($('#swapNeutralCost')?.checked){text(548,842,'✓',28)}
      const jpeg=c.toDataURL('image/jpeg',.96),bytes=Uint8Array.from(atob(jpeg.split(',')[1]),x=>x.charCodeAt(0));pages.push({width:c.width,height:c.height,bytes});
    };
    const makeEntry=(date,who)=>{if(swapMode==='roster')return{date:null,shift:swapFmtPE(date),start:'',finish:''};const code=who==='request'?$('#swapRequestShift')?.value||'':$('#swapPartnerShift')?.value||'';const t=swapTimes(date,code);return{date,shift:code,start:t[0]||'',finish:t[1]||''}};
    if(swapMode==='multi'){
      const re=swapDateValue('#swapRequestEndDate'),pe=swapDateValue('#swapPartnerEndDate'),n=Math.round((re-rd)/86400000)+1;
      for(let i=0;i<n;i++)addPage(makeEntry(new Date(rd.getTime()+i*86400000),'request'),makeEntry(new Date(pd.getTime()+i*86400000),'partner'));
    }else addPage(makeEntry(rd,'request'),makeEntry(pd,'partner'));
    const blob=swapJpegPdf(pages),url=URL.createObjectURL(blob),w=window.open(url,'_blank');if(!w){const a=document.createElement('a');a.href=url;a.download=`ShiftMate-Shift-Swap-${localISO(rd)}.pdf`;document.body.appendChild(a);a.click();a.remove()}setTimeout(()=>URL.revokeObjectURL(url),60000);closeShiftSwap();toast(pages.length>1?`${pages.length} shift swap forms generated`:'Shift swap form generated');
  }
