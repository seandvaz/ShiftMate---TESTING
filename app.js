...
(() => {
  ...
  function actualShiftPeriod(entry,date){
    ...
    // Highest-confidence grid-roster format:
    // Pay Period: 23/08/2026 to 05/09/2026
    let mm=norm.match(/PAY\s*PERIOD\s*:?\s*(\d{1,2})[\/.\-](\d{1,2})[\/.\-](\d{2,4})\s*(?:TO|[-–—])\s*(\d{1,2})[\/.\-](\d{1,2})[\/.\-](\d{2,4})/i);
    if(mm){
      const end=parseDMY(mm[4],mm[5],mm[6]);
      if(end)return fmt(end);
    }

    // Explicit PE / PERIOD END formats.
    mm=norm.match(/(?:PERIOD\s*END|\bPE\b)\s*:?\s*(\d{1,2})[\/.\-](\d{1,2})[\/.\-](\d{2,4})/i);
    if(mm){
      const d=parseDMY(mm[1],mm[2],mm[3]);
      if(d)return fmt(d);
    }

    // Actuals roster: written dates such as "Sun, 19 April 2026".
    const months={JANUARY:0,FEBRUARY:1,MARCH:2,APRIL:3,MAY:4,JUNE:5,JULY:6,AUGUST:7,SEPTEMBER:8,OCTOBER:9,NOVEMBER:10,DECEMBER:11,
                  JAN:0,FEB:1,MAR:2,APR:3,JUN:5,JUL:6,AUG:7,SEP:8,SEPT:8,OCT:9,NOV:10,DEC:11};
    const found=[];
    const rx=/(?:MON|TUE|WED|THU|FRI|SAT|SUN)[A-Z]*\s*,?\s*(\d{1,2})\s+([A-Z]{3,9})\s+(\d{4})/ig;
    let dm;
    while((dm=rx.exec(norm))){
      const mon=months[String(dm[2]).toUpperCase()];
    ...
    if(found.length>=14){
      ...
      for(const d of found){
        if(!uniq.length||uniq[uniq.length-1].getTime()!==d.getTime())uniq.push(d);
      }
      // Require a complete 14-day run; use the final date only if unambiguous.
      for(let i=0;i<=uniq.length-14;i++){
        let ok=true;
        for(let j=1;j<14;j++){
          const prev=new Date(uniq[i+j-1]);prev.setDate(prev.getDate()+1);
          if(prev.toDateString()!==uniq[i+j].toDateString()){ok=false;break;}
        }
        if(ok)return fmt(uniq[i+13]);
      }
    }

    // Preserve old detector body as fallback.
