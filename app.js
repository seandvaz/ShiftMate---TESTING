...
const perthShiftLabels={PN:'Perth Assist Arvo',PA:'Perth Afternoon',PD:'Perth Assist Day',PM:'Perth Morning',PRD:'Perth Roaming Day',UD:'Underground Day',UN:'Underground Arvo'};
  ...
  function selectSwapDate(date){
    ...
    if(swapMode==='multi'){
      if(!swapState.requestDates.some(d=>localISO(d)===key))swapState.requestDates.push(date);
      if(!swapState.partnerDates.some(d=>localISO(d)===key))swapState.partnerDates.push(date);
      renderSwapEntries();
    }else{
  ...
  const syncSwapMode=()=>{
    ...
    $('#swapSingleFields')?.toggleAttribute('hidden',multi);$('#swapMultiFields')?.toggleAttribute('hidden',!multi);
    $('#swapRosterNote')?.toggleAttribute('hidden',!roster);$('#swapCalendarWrap')?.toggleAttribute('hidden',false);
    if(roster){const d=swapDateValue('#swapRequestDate')||new Date();const pe=swapFmtPE(d);$('#swapRequestShift').innerHTML=`<option value="${pe}">${pe}</option>`;$('#swapPartnerShift').innerHTML=`<option value="${pe}">${pe}</option>`;$('#swapRequestStart').value='';$('#swapRequestFinish').value='';$('#swapPartnerStart').value='';$('#swapPartnerFinish').value='';}
    else if(!multi)syncSwapTimes();
    else renderSwapEntries();
  };
  const openShiftSwap=()=>{
  ...
  async function generateShiftSwap(){
    ...
    if(swapMode==='multi'){
      ...
      const cards=$$('#swapMultiEntries .swap-entry-card');cards.forEach((card,i)=>{const date=swapState.requestDates[i],rq=card.querySelector('[data-swap-side="request"].swap-entry-shift')?.value||'',ps=card.querySelector('[data-swap-side="partner"].swap-entry-shift')?.value||'';entries.push({request:{date,shift:rq,start:card.querySelector('[data-swap-side="request"].swap-entry-start')?.value||'',finish:card.querySelector('[data-swap-side="request"].swap-entry-finish')?.value||''},partner:{date,shift:ps,start:card.querySelector('[data-swap-side="partner"].swap-entry-start')?.value||'',finish:card.querySelector('[data-swap-side="partner"].swap-entry-finish')?.value||''}})});
    ...
    try{
      const template=await new Promise((res,rej)=>{const im=new Image();im.onload=()=>res(im);im.onerror=()=>rej(new Error('Template failed to load'));im.src='./shift-change-form-template.png'});
      const pages=[];
      entries.forEach(({request:a,partner:b})=>{
        const c=document.createElement('canvas');c.width=1241;c.height=1754;
        const ctx=c.getContext('2d');ctx.drawImage(template,0,0);ctx.textBaseline='middle';ctx.fillStyle='#111';
        const text=(x,y,v,size=25,bold=true)=>{if(v===undefined||v===null||v==='')return;ctx.font=`${bold?'bold ':''}${size}px Arial`;ctx.fillText(String(v),x,y)};
        const clearValue=(x1,y1,x2,y2)=>{ctx.fillStyle='#fff';ctx.fillRect(x1,y1,x2-x1,y2-y1);ctx.fillStyle='#111'};
        const drawValue=(x,y,v,size=25)=>{if(v===undefined||v===null||v==='')return;ctx.font=`bold ${size}px Arial`;ctx.fillText(String(v),x,y)};
        // Clear only the pre-filled sample values, leaving every label, line and explanatory paragraph intact.
        const sampleAreas=[
          [210,238,405,285],[770,238,920,285],[205,285,430,315],[690,285,930,315],
          [275,340,490,380],[820,340,1110,380],[275,390,490,425],[820,390,1110,425],
          [275,440,490,480],[820,440,1110,480],[275,490,490,530],[820,490,1110,530],
          [205,1165,430,1210],[770,1165,930,1210],[205,1205,430,1235],[690,1205,930,1235],
          [275,1245,590,1285],[820,1245,1110,1285],[275,1295,590,1330],[820,1295,1110,1330],
          [275,1345,590,1380],[820,1345,1110,1380],[275,1395,590,1430],[820,1395,1110,1430]
        ];
        sampleAreas.forEach(r=>clearValue(...r));
        // Requesting employee: left = rostered/original shift, right = requested/new shift.
        drawValue(215,265,reqName,25);drawValue(790,265,reqService,25);
        drawValue(280,305,a.shift,22);drawValue(700,305,b.shift,22);
        drawValue(285,365,a.date?swapFmtDate(a.date):'',24);drawValue(830,365,b.date?swapFmtDate(b.date):'',24);
        drawValue(285,415,a.shift,25);drawValue(830,415,b.shift,25);
        drawValue(285,465,a.start,25);drawValue(830,465,b.start,25);
        drawValue(285,515,a.finish,25);drawValue(830,515,b.finish,25);
        // Replacement employee: left = rostered/original shift, right = requested/new shift.
        drawValue(215,1195,partnerName,25);drawValue(790,1195,partnerService,25);
        drawValue(280,1230,b.shift,22);drawValue(700,1230,a.shift,22);
        drawValue(285,1275,b.date?swapFmtDate(b.date):'',24);drawValue(830,1275,a.date?swapFmtDate(a.date):'',24);
        drawValue(285,1320,b.shift,25);drawValue(830,1320,a.shift,25);
        drawValue(285,1370,b.start,25);drawValue(830,1370,a.start,25);
        drawValue(285,1420,b.finish,25);drawValue(830,1420,a.finish,25);
        if($('#swapContingent')?.checked)drawValue(600,584,'✓',28);
        if($('#swapNeutralCost')?.checked)drawValue(548,842,'✓',28);
...
const syncViewport=()=>{const vv=window.visualViewport;document.documentElement.style.setProperty('--app-vh',`${vv?.height||window.innerHeight}px`);const nav=document.querySelector('.bottom-nav');if(nav)document.documentElement.style.setProperty('--bottom-nav-h',`${Math.ceil(nav.getBoundingClientRect().height)}px`);requestAnimationFrame(()=>{if(nav){nav.style.bottom='0px';nav.style.left='0px';nav.style.right='0px'}sizeCalendarViewport()})};
  ...
  $$('#shiftSwapSheet [data-swap-mode]').forEach(b=>b.onclick=()=>{swapMode=b.dataset.swapMode;$$('#shiftSwapSheet [data-swap-mode]').forEach(x=>x.classList.toggle('active',x===b));if(swapMode==='multi'&&!swapState.requestDates.length){const d=swapDateValue('#swapRequestDate')||new Date();swapState.requestDates=[d];}syncSwapMode()});
  ...
  if($('#addSwapDate'))$('#addSwapDate').onclick=()=>{const last=swapState.requestDates.at(-1)||swapDateValue('#swapRequestDate')||new Date();const next=new Date(last);next.setDate(next.getDate()+1);if(!swapState.requestDates.some(d=>localISO(d)===localISO(next)))swapState.requestDates.push(next);renderSwapEntries();renderSwapCalendar(localISO(next));};
