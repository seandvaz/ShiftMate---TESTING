...
function installDismissibleSheets(){
  ...
  if(swapSheetCard){
    let swapStartY=0,swapStartX=0,swapDragging=false;
    swapSheetCard.addEventListener('touchstart',e=>{
      ...
      swapStartY=e.touches[0].clientY;swapStartX=e.touches[0].clientX;swapDragging=true;
    },{passive:true});
    swapSheetCard.addEventListener('touchend',e=>{
      if(!swapDragging)return;swapDragging=false;
      const t=e.changedTouches[0],dy=t.clientY-swapStartY,dx=Math.abs(t.clientX-swapStartX);
      if(dy>85&&dx<60&&(swapSheetCard.scrollTop||0)<=2)closeShiftSwap();
    },{passive:true});
    swapSheetCard.addEventListener('touchcancel',()=>{swapDragging=false},{passive:true});
  }
const syncViewport=()=>{const vv=window.visualViewport;document.documentElement.style.setProperty('--app-vh',`${vv?.height||window.innerHeight}px`);const nav=document.querySelector('.bottom-nav');if(nav)document.documentElement.style.setProperty('--bottom-nav-h',`${Math.ceil(nav.getBoundingClientRect().height)}px`);requestAnimationFrame(()=>{if(nav){nav.style.bottom='0px';nav.style.left='0px';nav.style.right='0px'}sizeCalendarViewport()})};
  ...
  if($('#swapRequestDate'))$('#swapRequestDate').onchange=()=>{syncSwapMode()};
  if($('#swapPartnerDate'))$('#swapPartnerDate').onchange=()=>{syncSwapMode()};
  if($('#swapRequestEndDate'))$('#swapRequestEndDate').onchange=()=>{};
  if($('#swapPartnerEndDate'))$('#swapPartnerEndDate').onchange=()=>{};
