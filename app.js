...
(() => {
  ...
  function generateLeaveForm(block){
    ...
    $$('.leave-group-hours').forEach(input=>{const g=groups.find(x=>x.key===input.dataset.groupKey),v=Number(input.value);if(g&&Number.isFinite(v))g.hours=Math.max(0,v)});
    ...
    const pageFor=(g,index)=>{
      const rowName=leaveFormRow(g),first=g.entries[0]?.date||adjustedStart,last=g.entries[g.entries.length-1]?.date||adjustedEnd;
      const bodyRows=rows.map(r=>`<tr><td>${esc(r)}</td><td>${r===rowName?fmtFormDate(first):''}</td><td>${r===rowName?fmtFormDate(last):''}</td><td>${r===rowName?g.hours.toFixed(1):''}</td><td>${r===rowName?esc(comments):''}</td></tr>`).join('');
      return `<section class="leave-page${index?' page-break':''}"><div style="text-align:center;font-size:10px">OFFICIAL</div><h1>APPLICATION FOR LEAVE – TRANSIT OFFICERS</h1><div class="sub">Once completed please return to HR Services - Personnel and Payroll</div>
      ...
      <div class="section">SECTION 4: ENDORSEMENT &amp; APPROVALS</div><table class="endorse"><tr><td><b>Leave Approved:</b><br>Yes ☐ &nbsp; No ☐</td><td><b>Comments:</b></td></tr><tr><th>Employee</th><th>Supervisor / Manager</th></tr><tr><td><b>Name:</b> ${esc(name)}</td><td><b>Name:</b></td></tr><tr><td><b>Signature:</b></td><td><b>Signature:</b></td></tr><tr><td><b>Date:</b> ${fmtFormDate(today)}</td><td></td></tr><tr><th>Roster Office Use Only</th><th>Roster Clerk / Checking Officer</th></tr><tr><td><b>Name:</b></td><td><b>Name:</b></td></tr><tr><td><b>Signature:</b></td><td><b>Signature:</b></td></tr></table><div style="text-align:right;margin-top:8px;font-size:9px">OFFICIAL · Effective as of 1 July 2026</div></section>`;
    };
    const html=`<!doctype html><html><head><meta charset="utf-8"><title>Leave Application - ${esc(name||'Transit Officer')}</title><style>@page{size:A4;margin:10mm}body{font-family:Arial,sans-serif;color:#111;margin:0;font-size:11px}.toolbar{margin-bottom:10px}.toolbar button{padding:8px 14px;margin-right:8px}h1{text-align:center;font-size:19px;margin:4px 0}.sub{text-align:center;color:#267f95;font-weight:bold;margin-bottom:8px}.section{background:#2f8ca1;color:white;font-weight:bold;padding:4px 6px;margin-top:6px}table{width:100%;border-collapse:collapse}td,th{border:1px solid #777;padding:4px;vertical-align:top}th{font-size:10px}.label{font-weight:bold}.checks{display:flex;gap:30px;padding:8px;border:1px solid #777}.endorse td{height:18px}.page-break{break-before:page;page-break-before:always}@media print{.toolbar{display:none}}</style></head><body><div class="toolbar"><button onclick="window.print()">Print / Save PDF</button><button onclick="window.close()">Close</button></div>${groups.map(pageFor).join('')}</body></html>`;
    ...
    $('#leaveFormSheet').hidden=true;pendingLeaveDocument=null;toast(groups.length>1?`${groups.length} separate leave forms generated`:'Leave form generated');
  }
