...
(() => {
  ...
  const opts=(selected,line,date=null)=>{
    ...
    return codes.map(code=>`<option value="${code}" ${code===selected?'selected':''}>${code?SHIFT_DATA[code].name:'Off / no shift'}</option>`).join('');
  };
  ...
  function applyShiftDefaults(card,date,force=false){
    ...
    if(face)face.textContent=code||'OFF';
    const data=SHIFT_DATA[code];
  ...
  function buildRoster(){
    ...
    current.days.forEach((row,i)=>{
      ...
      <div class="day-main">
        <label class="shift-picker"><span class="shift-face-code">${row.code||'OFF'}</span><select class="shift-code" aria-label="Select shift">${opts(row.code,initialLine,date)}</select></label>
      ...
      card.querySelector('.details-button').onclick=()=>card.classList.add('open');
      ...
      card.querySelector('.shift-code').onchange=()=>{
        ...
        const face=card.querySelector('.shift-face-code');
        if(face)face.textContent=card.querySelector('.shift-code').value||'OFF';
  ...
  const syncViewport=()=>{document.documentElement.style.setProperty('--app-vh',`${window.visualViewport?.height||window.innerHeight}px`);requestAnimationFrame(sizeCalendarViewport)};
  window.addEventListener('resize',syncViewport,{passive:true});window.addEventListener('pageshow',syncViewport,{passive:true});document.addEventListener('visibilitychange',()=>{if(!document.hidden)syncViewport()});window.visualViewport?.addEventListener('resize',syncViewport,{passive:true});requestAnimationFrame(syncViewport);
