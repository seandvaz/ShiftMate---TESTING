...
(() => {
  ...
  function activeDashboardCycle(){
    if(selectedPayCycleId){
      const selected=cycleById(selectedPayCycleId);
      if(selected)return selected;
      selectedPayCycleId=null;
    }
    return upcomingPayCycle();
  }
...
const perthShiftLabels={PN:'Perth Assist Arvo',PA:'Perth Afternoon',PD:'Perth Assist Day',PM:'Perth Morning',PRD:'Perth Roaming Day',UD:'Underground Day',UN:'Underground Arvo'};
  ...
  const swapPEForDate=date=>{
    if(!date)return '';
    const cycle=swapCycle(date);
    if(cycle?.startDate){const end=addRosterDays(parseDate(cycle.startDate),13);return swapFmtPE(end)}
    const projectedStart=projectionFortnightStartForProfile(date);
    if(projectedStart)return swapFmtPE(addRosterDays(projectedStart,13));
    return '';
  };
...
const syncViewport=()=>{const vv=window.visualViewport;document.documentElement.style.setProperty('--app-vh',`${vv?.height||window.innerHeight}px`);const nav=document.querySelector('.bottom-nav');if(nav)document.documentElement.style.setProperty('--bottom-nav-h',`${Math.ceil(nav.getBoundingClientRect().height)}px`);requestAnimationFrame(()=>{if(nav){nav.style.bottom='0px';nav.style.left='0px';nav.style.right='0px'}sizeCalendarViewport()})};
  window.addEventListener('resize',syncViewport,{passive:true});window.addEventListener('orientationchange',syncViewport,{passive:true});window.addEventListener('pageshow',e=>{syncViewport();if(e.persisted)renderHomeDashboard()},{passive:true});document.addEventListener('visibilitychange',()=>{if(!document.hidden){syncViewport();renderHomeDashboard()}});window.visualViewport?.addEventListener('resize',syncViewport,{passive:true});window.visualViewport?.addEventListener('scroll',syncViewport,{passive:true});installDismissibleSheets();
