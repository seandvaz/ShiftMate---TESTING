...
const perthShiftLabels={PN:'Perth Assist Arvo',PA:'Perth Afternoon',PD:'Perth Assist Day',PM:'Perth Morning',PRD:'Perth Roaming Day',UD:'Underground Day',UN:'Underground Arvo'};
  ...
  async function openRosterScanner(mode='current'){
    ...
    try{
      ...
      const video=$('#rosterScanVideo');video.srcObject=rosterScanStream;await video.play();status.textContent='Fit the full fortnight roster inside the landscape scan area, then tap Scan Roster.';
  ...
  function detectPeriodEnd(text,words=[]){
    ...
    const clean=v=>String(v||'').replace(/[Oo]/g,'0').replace(/[Il]/g,'1');
    const valid=(d,m,y)=>{
      d=Number(clean(d));m=Number(clean(m));y=Number(clean(y));
      if(y<100)y+=2000;
      const x=new Date(y,m-1,d);
      return x.getFullYear()===y&&x.getMonth()===m-1&&x.getDate()===d?x:null;
    };
    const add=d=>{if(d&&!Number.isNaN(d.getTime()))dates.push(new Date(d.getFullYear(),d.getMonth(),d.getDate()))};

    // Grid roster: "Pay Period: 23/08/2026 to 05/09/2026".
    ...
    if(m){const pe=valid(m[4],m[5],m[6]);if(pe)return pe}

    // Explicit PE / Period End.
    m=norm.match(/(?:PERIOD\s*END|\bPE\b)\s*[:\-]?\s*([0-9OoIl]{1,2})\s*[\/.\-]?\s*([0-9OoIl]{1,2})\s*[\/.\-]?\s*([0-9OoIl]{2,4})/i);
    if(m){const pe=valid(m[1],m[2],m[3]);if(pe)return pe}
    m=norm.match(/\bPE\s*([0-9OoIl]{2})([0-9OoIl]{2})([0-9OoIl]{2,4})\b/i);
    if(m){const pe=valid(m[1],m[2],m[3]);if(pe)return pe}

    const years=[...norm.matchAll(/\b(20\d{2})\b/g)].map(x=>Number(x[1]));
    const yearHint=years[0]||new Date().getFullYear();
    const dates=[];

    // Numeric dates from OCR text.
    for(const x of norm.matchAll(/\b([0-9OoIl]{1,2})\s*[\/.\-]\s*([0-9OoIl]{1,2})(?:\s*[\/.\-]\s*([0-9OoIl]{2,4}))?\b/g)){
      add(valid(x[1],x[2],x[3]||yearHint));
    }
    // Numeric dates from individual OCR words.
    (words||[]).forEach(w=>{
      const x=String(w?.text||'').match(/([0-9OoIl]{1,2})[\/.\-]([0-9OoIl]{1,2})(?:[\/.\-]([0-9OoIl]{2,4}))?/);
      if(x)add(valid(x[1],x[2],x[3]||yearHint));
    });

    // Actuals roster: "Sun, 19 April 2026".
    const months={JANUARY:0,FEBRUARY:1,MARCH:2,APRIL:3,MAY:4,JUNE:5,JULY:6,AUGUST:7,SEPTEMBER:8,OCTOBER:9,NOVEMBER:10,DECEMBER:11,
                  JAN:0,FEB:1,MAR:2,APR:3,JUN:5,JUL:6,AUG:7,SEP:8,SEPT:8,OCT:9,NOV:10,DEC:11};
    for(const x of norm.matchAll(/(?:MON|TUE|WED|THU|FRI|SAT|SUN)[A-Z]*\s*,?\s*([0-9OoIl]{1,2})\s+([A-Z]{3,9})\s+(20\d{2})/ig)){
      const mon=months[String(x[2]).toUpperCase()];
      if(mon!==undefined)add(valid(x[1],mon+1,x[3]));
    }

    // Mandatory confidence gate: only derive PE from a complete consecutive 14-day run.
    const uniq=[...new Map(dates.sort((a,b)=>a-b).map(d=>[localISO(d),d])).values()];
    for(let i=0;i<=uniq.length-14;i++){
      let ok=true;
      for(let j=1;j<14;j++){
        if(localISO(addRosterDays(uniq[i+j-1],1))!==localISO(uniq[i+j])){ok=false;break}
      }
      if(ok)return uniq[i+13];
    }
    return null;
  }
  function scanAssignmentsFromWords(words,periodEnd){
    ...
    // Actuals/list roster fallback: group OCR words into visual rows, recognise
    // the written date, then select only a valid ShiftMate shift code on that row.
    if(assign.filter(Boolean).length<3){
      const monthMap={JANUARY:0,FEBRUARY:1,MARCH:2,APRIL:3,MAY:4,JUNE:5,JULY:6,AUGUST:7,SEPTEMBER:8,OCTOBER:9,NOVEMBER:10,DECEMBER:11,
                      JAN:0,FEB:1,MAR:2,APR:3,JUN:5,JUL:6,AUG:7,SEP:8,SEPT:8,OCT:9,NOV:10,DEC:11};
      const rows=[];
      [...cleanWords].sort((a,b)=>((a.box.y0+a.box.y1)/2)-((b.box.y0+b.box.y1)/2)).forEach(w=>{
        const cy=(w.box.y0+w.box.y1)/2;
        let row=rows.find(r=>Math.abs(r.y-cy)<16);
        if(!row){row={y:cy,words:[]};rows.push(row)}
        row.words.push(w);
        row.y=row.words.reduce((s,x)=>s+(x.box.y0+x.box.y1)/2,0)/row.words.length;
      });
      rows.forEach(row=>{
        row.words.sort((a,b)=>a.box.x0-b.box.x0);
        const line=row.words.map(w=>String(w.text||'')).join(' ').replace(/\s+/g,' ').trim();
        const dm=line.match(/(?:MON|TUE|WED|THU|FRI|SAT|SUN)[A-Z]*\s*,?\s*(\d{1,2})\s+([A-Z]{3,9})\s+(20\d{2})/i);
        if(!dm)return;
        const mon=monthMap[String(dm[2]).toUpperCase()];if(mon===undefined)return;
        const d=new Date(Number(dm[3]),mon,Number(dm[1]));
        ...
        if(idx<0||idx>13)return;
        const code=row.words.find(w=>valid.has(w.token));
        if(code)assign[idx]=code.token;
      });
    }
  ...
  async function captureRosterScan(){
    const video=$('#rosterScanVideo'),canvas=$('#rosterScanCanvas'),status=$('#rosterScanStatus');
    if(!video?.videoWidth){toast('Camera is not ready');return}

    const maxW=2400,scale=Math.min(1,maxW/video.videoWidth);
    canvas.width=Math.round(video.videoWidth*scale);
    canvas.height=Math.round(video.videoHeight*scale);
    const ctx=canvas.getContext('2d',{willReadFrequently:true});
    ctx.drawImage(video,0,0,canvas.width,canvas.height);

    // PC-monitor scan preprocessing: grayscale + moderate contrast to reduce colour moiré.
    const image=ctx.getImageData(0,0,canvas.width,canvas.height),px=image.data;
    for(let i=0;i<px.length;i+=4){
      const y=.299*px[i]+.587*px[i+1]+.114*px[i+2];
      const v=Math.max(0,Math.min(255,(y-128)*1.35+140));
    ...
    try{
      ...
      const result=await T.recognize(canvas,'eng',{logger:m=>{
        if(m.status==='recognizing text')status.textContent=`Reading roster… ${Math.round((m.progress||0)*100)}%`;
      }});
      const data=result.data||{},words=data.words||[];
      window.__shiftMateLastRosterOcr=String(data.text||''); // TEST diagnostic
      const pe=detectPeriodEnd(data.text,words);
      if(!pe){
        const chars=String(data.text||'').replace(/\s+/g,' ').trim().length;
        status.textContent=`Period End could not be detected. OCR captured ${chars} characters. No shifts were changed.`;
        return;
      }
      const assignments=scanAssignmentsFromWords(words,pe);
      pendingRosterScan={periodEnd:pe,assignments};
      closeRosterScanner();
      $('#rosterPeConfirmText').textContent=`Confirm PE: ${pe.toLocaleDateString('en-AU',{day:'numeric',month:'long',year:'numeric'})}`;
