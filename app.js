...
(() => {
  ...
  function reconcileHomeCurrent(){
    const stored=AppStorage.loadCurrent();
    if(!stored?.startDate||!Array.isArray(stored.days))return;
    const memEntered=(current?.days||[]).filter(d=>(d?.entered??Boolean(d?.code))&&d?.code).length;
    const storedEntered=stored.days.filter(d=>(d?.entered??Boolean(d?.code))&&d?.code).length;
    if(!current?.startDate || stored.startDate!==current.startDate || storedEntered>memEntered){
      current=JSON.parse(JSON.stringify(stored));
      current.days=current.days.map(row=>({...emptyDay(),...row,entered:Boolean(row.entered??row.code)}));
    }
  }
  function renderHomeDashboard(){
    reconcileHomeCurrent();
...
const syncViewport=()=>{const vv=window.visualViewport;document.documentElement.style.setProperty('--app-vh',`${vv?.height||window.innerHeight}px`);const nav=document.querySelector('.bottom-nav');if(nav)document.documentElement.style.setProperty('--bottom-nav-h',`${Math.ceil(nav.getBoundingClientRect().height)}px`);requestAnimationFrame(()=>{if(nav){nav.style.bottom='0px';nav.style.left='0px';nav.style.right='0px'}sizeCalendarViewport()})};
  window.addEventListener('pageshow',()=>{reconcileHomeCurrent();renderHomeDashboard()},{passive:true});
  document.addEventListener('visibilitychange',()=>{if(!document.hidden){reconcileHomeCurrent();renderHomeDashboard()} });
  ...
  // Shift Swap: swipe down from the sheet header to dismiss, matching Leave Form.
  ...
  if(swapSheetCard){
    ...
    swapSheetCard.addEventListener('touchend',e=>{
